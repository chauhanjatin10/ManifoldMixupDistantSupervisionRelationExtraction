...
class Batchify:
    def __init__(self, batch_size, instance_iterator):
        self.batch_size = batch_size
        self.instance_iterator = instance_iterator

    def __iter__(self):
        current_batch = []
        for inst in self.instance_iterator:
            if len(current_batch) < self.batch_size:
                current_batch.append([inst['sentence'].tokens, inst['label'].label])
            else:
                yield current_batch
                current_batch = []
                current_batch.append([inst['sentence'].tokens, inst['label'].label])
